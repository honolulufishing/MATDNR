...
function [u,loss,gen,bus,Lnbr_output,trsfm,shtc,shtr,vctr,elapsed_time, SOC_value,num_branch,flag] = opti_DNR_EDF_SCF_McCormick(gen,bus,Lnbr_all,trsfm,shtc,shtr,vctr,sysdt)
% Exact DistFlow-based Topology Reconfiguration Model using McCormick Linerization Method
% Obj: Minimize Real Power Loss over reconfigurable topology and reactive power compensation devices
% s.t. Spanning Tree (ST) Constraints + Single-Commodity Flow (SCF) Constraints + DistFlow equations using McCormick Linerization Relxations
...
A_auxiliary_1 = zeros(nLnbr,2*nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1 :nLnbr
...
A_auxiliary_2 = zeros(nLnbr,2*nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1 : nLnbr
...
U_P_var_1 = zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
...
for i = 1 :nLnbr
    ...
    if Lnbr_all(i,7)~= 2
        U_P_var_1(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+i)  =-1;  % y1
...
S_max = nb;
A_f_u_1 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
A_f_u_2 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1: nLnbr
    A_f_u_1(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+i) = 1;
    A_f_u_2(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+i) = 1;
end
A_f_u_1(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_f_u_2(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

A_beta_1 = zeros(nLnbr,2*nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1:nLnbr
    A_beta_1(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr+(i-1)*2+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr+2*i) =[1 1];
    A_beta_1(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+i) =-1;
end
...
A_beta = [zeros(nb,nLnbr+nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr) A_f zeros(nb,3*nLnbr);
    A_beta_1 ;
    zeros(n_source,nLnbr+nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr) A_beta_2 zeros(n_source,nLnbr);
    zeros(2*n_ex,nLnbr+nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr) A_beta_3 zeros(2*n_ex,nLnbr);
    zeros(nb-1,nLnbr+nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr) A_beta_4 zeros(nb-1,nLnbr);
    ];

B_beta = [b_f;b_beta_1 ; b_beta_2 ;b_beta_3; b_beta_4 ];
% -------------END------------------ %

%----Form the constant matrix for Aeq*x=beq-----%
Aeq_modify = [ (1/2).*A_P_reduced       zeros(nb-1,nb+npv+nshtc+2*nLnbr+nLnbr)       zeros(nb-1,nLnbr*4);
    (1/2).*A_Q_reduced       zeros(nb-1,nb)    A_UQG A_SHTC  zeros(nb-1,2*nLnbr+nLnbr+nLnbr*4);
    A_beta ;
    A_auxiliary_1;
    A_auxiliary_2;
    U_P_var_1;];

Beq_modify = [delta_P_s;delta_Q_s;B_beta;zeros(2*nLnbr,1);b_P_var_1];
% ------------- END ------------------ %
...
% McCormick Linearization Method for y1
A_y_1 = zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
A_y_2 = zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
b_y_1 = zeros(nLnbr,1);b_y_2 = zeros(nLnbr,1);
for i = 1 : nLnbr  
    if Lnbr_all(i,7)~= 2
        A_y_1(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+i) = 1;
        A_y_1(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+i) = bus(Lnbr_all(i,2),10).^2 - bus(Lnbr_all(i,3),9).^2;
      ...
      A_y_1(i,3*nLnbr+ P_index(i,1))  = -1;
        ...
        A_y_2(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+i) = 1;
        A_y_2(i,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+i) = -(bus(Lnbr_all(i,2),10).^2 - bus(Lnbr_all(i,3),9).^2);
        
          b_y_2(i,1) = -(bus(Lnbr_all(i,2),10).^2 - bus(Lnbr_all(i,3),9).^2);
    end
end

% inequality constraints for u^l*Smin<P^l,Q^l,I^l< u^l*Smax
S_max = 100;
A_P_u_1 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
A_P_u_2 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1: nLnbr
    A_P_u_1(i,i) = 1;
    A_P_u_2(i,i) = 1;
end
A_P_u_1(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr+2*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_P_u_2(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr+2*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

A_Q_u_1 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
A_Q_u_2 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1: nLnbr
    A_Q_u_1(i,nLnbr+i) = 1;
    A_Q_u_2(i,nLnbr+i) = 1;
end
A_Q_u_1(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr+2*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_Q_u_2(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+nLnbr+2*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

A_I_u_1 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
A_I_u_2 =zeros(nLnbr,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr);
for i = 1: nLnbr
    A_I_u_1(i,2*nLnbr+i) = 1;
    A_I_u_2(i,2*nLnbr+i) = 1;
end
A_I_u_1(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_I_u_2(:,3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

% Linear Constraints of QG 
% Reactive power capacity constraints for the reference bus 1
A_QGSW_inequal =  [zeros(1,nLnbr) 1 zeros(1,nLnbr-1) zeros(1,nLnbr) zeros(1,nb) zeros(1,npv) zeros(1,nshtc) zeros(1,2*nLnbr+nLnbr) zeros(1,4*nLnbr)];
...
% Bounds
lb = [-100.*ones(1,3*nLnbr) (bus(:,9).^2)' gen(2:end,5)'  shtc(:,4)' -100.*ones(1,2*nLnbr)  -(1.1^2-0.9^2).*ones(1,nLnbr) -nb*2.*ones(1,nLnbr) zeros(1,2*nLnbr), zeros(1,nLnbr) ];
ub =[100.*ones(1,3*nLnbr) (bus(:,10).^2)' gen(2:end,4)'  shtc(:,3)'  100.*ones(1,2*nLnbr)   (1.1^2-0.9^2).*ones(1,nLnbr)  nb*2.*ones(1,nLnbr) ones(1,2*nLnbr) ones(1,nLnbr) ];
...
prob.c = [obj_1 zeros(1,nLnbr*2+nb+nshtc+npv+2*nLnbr+nLnbr+4*nLnbr) ];
...
prob.blx = lb;
prob.bux = ub;
prob.ints.sub=[3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+4*nLnbr];   % u is a set of 0-1 integer variable
...
u = X_Opt(3*nLnbr+nb+npv+nshtc+2*nLnbr+nLnbr+3*nLnbr+1:3*nLnbr+nb+nshtc+npv+2*nLnbr+nLnbr+4*nLnbr,1);   % u is a set of 0-1 integer variable
beta = X_Opt(3*nLnbr+nb+npv+nshtc+2*nLnbr+2*nLnbr+nLnbr+1:3*nLnbr+nb+npv+nshtc+2*nLnbr+2*nLnbr+3*nLnbr,1);   % u is a set of 0-1 integer variable

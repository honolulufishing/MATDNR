...
function [loss,gen,bus,Lnbr,trsfm,shtc,shtr,vctr,elapsed_time,SOC_value,I2] = opti_var_polyhedral_mosek(gen,bus,Lnbr,trsfm,shtc,shtr,vctr,sysdt)
...
A_auxiliary_1 = zeros(nLnbr,2*nLnbr+nLnbr+nb+npv+nshtc+5*nLnbr);
for i = 1 :nLnbr
...
A_auxiliary_2 = zeros(nLnbr,2*nLnbr+nLnbr+nb+npv+nshtc+5*nLnbr);
for i = 1 : nLnbr
...
Aeq_modify = [ (1/2).*A_P_reduced   zeros(nb-1,nb+npv+nshtc) zeros(nb-1,5*nLnbr) ;
   (1/2).*A_Q_reduced   zeros(nb-1,nb)   A_UQG A_SHTC zeros(nb-1,5*nLnbr); 
        U_P_var zeros(nb-1,npv+nshtc) zeros(nb-1,5*nLnbr);
            A_auxiliary_1 ;
          A_auxiliary_2 ];
...
lb = [-100.*ones(1,3*(nb-1)) (bus(:,9).^2)' gen(2:end,5)'  shtc(:,4)' zeros(1,nLnbr)   -100.*ones(1,nLnbr)  zeros(1,2*nLnbr+nLnbr)  ];
ub =[100.*ones(1,3*(nb-1)) (bus(:,10).^2)' gen(2:end,4)'  shtc(:,3)'   100.*ones(1,2*nLnbr) nb.*ones(1,2*nLnbr+nLnbr)];

% %---polyhedral cones-----%
n_approx = 30;  % 30
phy = 2*pi/n_approx;
A_s_upper = zeros(n_approx*nLnbr,nLnbr+nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+2*nLnbr+nLnbr);
...
A_s_lower = zeros(n_approx*nLnbr,nLnbr+nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr+2*nLnbr+nLnbr);
...
% Linear Constraints
% QG(SW节点的QG)上下限约束
A_QGSW_inequal =  [zeros(1,nLnbr) 1 zeros(1,nb-2) zeros(1,nLnbr) zeros(1,nb) zeros(1,npv) zeros(1,nshtc) zeros(1,5*nLnbr)];
...
prob.c = [1 zeros(1,(nb-1)*3+nb +nshtc+npv-1+5*nLnbr )];
prob.a = sparse(A);
prob.blc = rl;
prob.buc = ru;
prob.blx = lb;

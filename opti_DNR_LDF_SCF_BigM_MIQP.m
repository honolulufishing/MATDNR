...
function [u,loss,gen,bus,Lnbr_output,trsfm,shtc,shtr,vctr,elapsed_time,num_branch,flag] = opti_DNR_LDF_SCF_BigM_MIQP(gen,bus,Lnbr_all,trsfm,shtc,shtr,vctr,sysdt)
...
A_beta = [zeros(nb,nLnbr+nLnbr+nb+npv+nshtc) A_f zeros(nb,3*nLnbr);
    A_beta_1 ;
...
for i = 1 :nLnbr
    if Lnbr_all(i,7) == 2
        ...
        U_P_var_1(i,i) = 2*Lnbr_all(i,4);  % cause Pl is positive
...
for i = 1 :nLnbr
    if Lnbr_all(i,7) == 2
        ...
        U_P_var_2(i,i) = 2*Lnbr_all(i,4);
        U_P_var_2(i,i+nLnbr) = 2*Lnbr_all(i,5);
    end
...
prob.c = zeros(1,nLnbr+ nLnbr+nb+nshtc+npv+4*nLnbr) ;

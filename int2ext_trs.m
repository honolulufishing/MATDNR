function [gen,bus,Lnbr,trsfm,shtc,shtr,vctr] = int2ext_trs(i2e, gen,bus,Lnbr,trsfm,shtc,shtr,vctr,sysdt,loss)
...
% Output Bus Solution
disp('                                                                            ')
disp('   Reactive DistOPF Solution with SOC Constraints by Optimization Toolbox')
...
head =['                                                                                                                    '
       '   LnBR.  Bus   Bus    R      X    1/2 B      Power Flow        Power Flow           Loss          VolMag   VolMag  '
       '    NO.  from   to    p.u.   p.u.   p.u.        f to t            t to f             p.u.            f        t     '
       '                                                                                                                    '];
disp(head)
...
for i = 1:size(Lnbr,1)
    ...
    fprintf(' %8.3f', Lnbr(i,11)), fprintf(' %8.3f\n', Lnbr(i,12)),
end
% fprintf('      \n'), fprintf('    Total                                ')
...
for i = 1:size(trsfm,1)
    ...
    fprintf(' %6.3f', trsfm(i,5)),fprintf(' %5.2f',trsfm(i,6)),

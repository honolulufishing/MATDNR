...
function [u,loss,gen,bus,Lnbr_output,trsfm,shtc,shtr,vctr,elapsed_time,num_branch,flag] = opti_DNR_LDF_SCF_McCormick_MIQP(gen,bus,Lnbr_all,trsfm,shtc,shtr,vctr,sysdt)
% Linearized DistFlow-based Topology Reconfiguration Model using McCormick Method
% Obj: Minimize Real Power Loss over reconfigurable topology and reactive power compensation devices
% s.t. Spanning Tree (ST) Constraints + Single-Commodity Flow (SCF) Constraints + Linearized DistFlow equations (1b) using McCormick
...
U_P_var_1 = zeros(nLnbr,2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr);
b_P_var_1 = zeros(nLnbr,1);
for i = 1 :nLnbr
    
    if Lnbr_all(i,7) == 2
        U_P_var_1(i,2*nLnbr+ P_index(i,1))  = 1;
        U_P_var_1(i,2*nLnbr+ P_index(i,2))  = -1;
        
        U_P_var_1(i,i) = -2*Lnbr_all(i,4);  % equal to 2PR and note that cause Pl is positive
        U_P_var_1(i,i+nLnbr) = -2*Lnbr_all(i,5);
       
    end
    
    if Lnbr_all(i,7)~= 2
        U_P_var_1(i,2*nLnbr+nb+npv+nshtc+i)  =1;  % y1
...
S_max = nb;
A_f_u_1 =zeros(nLnbr,2*nLnbr+nb+npv+nshtc+5*nLnbr);
A_f_u_2 =zeros(nLnbr,2*nLnbr+nb+npv+nshtc+5*nLnbr);
for i = 1: nLnbr
    A_f_u_1(i,2*nLnbr+nb+npv+nshtc+nLnbr+i) = 1;
    A_f_u_2(i,2*nLnbr+nb+npv+nshtc+nLnbr+i) = 1;
end
A_f_u_1(:,2*nLnbr+nb+npv+nshtc+nLnbr+3*nLnbr+1:2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_f_u_2(:,2*nLnbr+nb+npv+nshtc+nLnbr+3*nLnbr+1:2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

A_beta_1 = zeros(nLnbr,2*nLnbr+nb+npv+nshtc+5*nLnbr);
for i = 1:nLnbr
    A_beta_1(i,2*nLnbr+nb+npv+nshtc+nLnbr+nLnbr+(i-1)*2+1:2*nLnbr+nb+npv+nshtc+nLnbr+nLnbr+2*i) =[1 1];
    A_beta_1(i,2*nLnbr+nb+npv+nshtc+4*nLnbr+i) =-1;
end
...
A_beta = [zeros(nb,nLnbr+nLnbr+nb+npv+nshtc+nLnbr) A_f zeros(nb,3*nLnbr);
    A_beta_1 ;
    zeros(n_source,nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr) A_beta_2 zeros(n_source,nLnbr);
    zeros(2*n_ex,nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr) A_beta_3 zeros(2*n_ex,nLnbr);
    zeros(nb-1,nLnbr+nLnbr+nb+npv+nshtc+2*nLnbr) A_beta_4 zeros(nb-1,nLnbr);
    ];
...
Aeq_modify = [ A_P_reduced       zeros(nb-1,nb+npv+nshtc)       zeros(nb-1,nLnbr*5);
    A_Q_reduced       zeros(nb-1,nb)    A_UQG A_SHTC  zeros(nb-1,nLnbr*5);
    A_beta ;
     U_P_var_1;];

Beq_modify = [delta_P_s;delta_Q_s;B_beta;b_P_var_1];
% ------------- END ------------------ %

% ---------Inequality constraints to form matrix A and vector b------------%
% McCormick Linearization Method for y1
A_y_1 = zeros(nLnbr,2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr);
A_y_2 = zeros(nLnbr,2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr);
b_y_1 = zeros(nLnbr,1);b_y_2 = zeros(nLnbr,1);
for i = 1 : nLnbr  
    if Lnbr_all(i,7)~= 2
        A_y_1(i,2*nLnbr+nb+npv+nshtc+i) = 1;
        A_y_1(i,2*nLnbr+nb+npv+nshtc+nLnbr+3*nLnbr+i) = bus(Lnbr_all(i,2),10).^2 - bus(Lnbr_all(i,3),9).^2;
        b_y_1(i,1) = bus(Lnbr_all(i,2),10).^2 - bus(Lnbr_all(i,3),9).^2;
        
      A_y_1(i,2*nLnbr+ P_index(i,1))  = -1;
        A_y_1(i,2*nLnbr+P_index(i,2))  = 1;
        
         A_y_2(i,2*nLnbr+P_index(i,1))  = -1;
        A_y_2(i,2*nLnbr+P_index(i,2))  = 1;
        
        A_y_2(i,2*nLnbr+nb+npv+nshtc+i) = 1;
        A_y_2(i,2*nLnbr+nb+npv+nshtc+nLnbr+3*nLnbr+i) = -(bus(Lnbr_all(i,2),10).^2 - bus(Lnbr_all(i,3),9).^2);
...
S_max = 100;
A_P_u_1 =zeros(nLnbr,2*nLnbr+nb+npv+nshtc+5*nLnbr);
A_P_u_2 =zeros(nLnbr,2*nLnbr+nb+npv+nshtc+5*nLnbr);
for i = 1: nLnbr
    A_P_u_1(i,i) = 1;
    A_P_u_2(i,i) = 1;
end
A_P_u_1(:,2*nLnbr+nb+npv+nshtc+nLnbr+nLnbr+2*nLnbr+1:2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_P_u_2(:,2*nLnbr+nb+npv+nshtc+nLnbr+nLnbr+2*nLnbr+1:2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

A_Q_u_1 =zeros(nLnbr,2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr);
A_Q_u_2 =zeros(nLnbr,2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr);
for i = 1: nLnbr
    A_Q_u_1(i,nLnbr+i) = 1;
    A_Q_u_2(i,nLnbr+i) = 1;
end
A_Q_u_1(:,2*nLnbr+nb+npv+nshtc+nLnbr+nLnbr+2*nLnbr+1:2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr)=-S_max*diag(ones(1,nLnbr));
A_Q_u_2(:,2*nLnbr+nb+npv+nshtc+nLnbr+nLnbr+2*nLnbr+1:2*nLnbr+nb+npv+nshtc+nLnbr+4*nLnbr)=S_max*diag(ones(1,nLnbr));

% Linear Constraints
% Reactive power capacity constraints for the reference bus 1
A_QGSW_inequal =  [zeros(1,nLnbr) 1 zeros(1,nLnbr-1) zeros(1,nb) zeros(1,npv) zeros(1,nshtc) zeros(1,5*nLnbr)];
...
% Bounds
lb = [-100.*ones(1,2*nLnbr) (bus(:,9).^2)' gen(2:end,5)'  shtc(:,4)' -(1.1^2-0.9^2).*ones(1,nLnbr) -nb*2.*ones(1,nLnbr) zeros(1,2*nLnbr), zeros(1,nLnbr) ];
ub =[100.*ones(1,2*nLnbr) (bus(:,10).^2)' gen(2:end,4)'  shtc(:,3)'   (1.1^2-0.9^2).*ones(1,nLnbr) nb*2.*ones(1,nLnbr) ones(1,2*nLnbr) ones(1,nLnbr) ];
...
prob.c = zeros(1,nLnbr+ nLnbr+nb+nshtc+npv+5*nLnbr) ;
% Specify the quadratic terms of the objective.
prob.qosubi = [1:nLnbr+ nLnbr]';
prob.qosubj = [1:nLnbr+ nLnbr]';
prob.qoval = 2.*[Lnbr_all(:,4);Lnbr_all(:,4)];
...
prob.blx = lb;
prob.bux = ub;
prob.ints.sub=[2*nLnbr+nb+npv+nshtc+4*nLnbr+1:2*nLnbr+nb+npv+nshtc+5*nLnbr];   % u is a set of 0-1 integer variable
...
u = X_Opt(2*nLnbr+nb+npv+nshtc+4*nLnbr+1:2*nLnbr+nb+nshtc+npv+5*nLnbr,1);   % u is a set of 0-1 integer variable
...
[gen,bus_exact,Lnbr_s,trsfm,shtc,shtr,vctr,loss_exact] = pf(gen,bus,Lnbr_all(u>0,1:6),trsfm,shtc,shtr,vctr,sysdt);
